...
#include "hho_params.hpp"   // hho_contact::trace_variant
...
/* Dispatcher: picks the face trace or the cell trace.  This is the ONLY
 * place the two Nitsche versions differ inside this file.                  */
template<typename Mesh, typename Basis>
Matrix<typename Mesh::coordinate_type, Dynamic, Dynamic>
eval_displacement_trace_operator(const Mesh&                      msh,
                                 const typename Mesh::cell_type&  cl,
                                 const Basis&                     fb,
                                 const size_t                     cell_degree,
                                 const typename Mesh::point_type& pt,
...
{
    if (variant == hho_contact::trace_variant::cell)
        return eval_cell_trace_operator(msh, cl, cell_degree, pt, num_total_dofs);

    return eval_trace_operator(msh, fb, pt, num_total_dofs, face_offset);
}
                            ...
                            const hho_contact::trace_variant variant = hho_contact::trace_variant::face)
{
    ...
    for (auto& fc : fcs)
    {
        ...
        if (bnd.is_contact_face(fc))
        {
            auto fb = make_vector_monomial_basis(msh, fc, fdi.degree());
            ...
            for (auto& qp : qps)
            {
                matrix_type traction_op = eval_traction_operator(msh, gb, grad_op,
                                                                 qp.point(), n,
                                                                 mu, lam);
                matrix_type trace_op    = eval_displacement_trace_operator(
                                        msh, cl, fb, cell_infos.cell_degree(),
                                        qp.point(), num_total_dofs, face_offset,
                                        variant);
